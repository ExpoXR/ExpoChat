...
> Container and service names retain the `ollma-` prefix for deployment compatibility.
...
For the first public GitHub release, create a new one-commit repository from the verified
working tree instead of exposing the private development history:

```bash
./scripts/prepare-public-release.sh /absolute/path/to/new-expochat-public
```

The command refuses existing destinations, excludes ignored runtime data, and runs the
release audit against both the exported tree and its clean Git history. It does not add a
remote, push, or change repository visibility.
